...
def demo_data() -> tuple[dict, dict]:
    ...
    profile = {
        ...
        "following": 0,
        "stars": 1,
        "languages": [["TypeScript", 3], ["JavaScript", 2], ["HTML", 1], ["Python", 1]],
    }
...
def identity_svg(cfg: dict) -> str:
    ...
    skills = "".join(f'<g transform="translate({337 + index * 87} 338)"><rect width="78" height="20" rx="10" fill="#0d3636" stroke="#17675f"/><text x="39" y="14" text-anchor="middle" class="chip">{escape(skill)}</text></g>' for index, skill in enumerate(cfg.get("skills", [])[:6]))
...
<text x="337" y="305" class="body">{escape(trim(cfg['role'], 58))} · {escape(cfg['location'])}</text><text x="337" y="319" class="mono">{escape(trim(cfg.get("status", ""), 72))}</text>{skills}
